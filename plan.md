...
### 추가 보완 9 (2026-06-12)
- 기본 선택 종목 `069500`이 빈 검색 결과 상위 200개 제한에 밀려 보이지 않는 문제를 보완하기 위해, `search_tickers`의 기본 정렬에 `TICKER='069500'` 우선순위를 추가했다.

### 추가 보완 10 (2026-06-12)
- Streamlit Lightweight Charts 차트에 Oracle 종가 기준 20일 이동평균선(MA20) 라인 시리즈를 추가했다.

### 추가 보완 11 (2026-06-12)
- 사용자 요청에 따라 Lightweight Charts 캔들/거래량 색상을 한국식 표기(상승 빨강, 하락 파랑)로 변경했다.

### 추가 보완 12 (2026-06-12)
- Streamlit Lightweight Charts 차트에 샹들리에 Exit 라인(기본 파라미터: 기간 22, ATR 배수 3.0)을 추가했다.

### 추가 보완 13 (2026-06-12)
- 샹들리에 Exit 파라미터를 화면에서 조절할 수 있도록 옵션 입력(CH Period, ATR Period, ATR Mult)을 추가했다.
- 사용자 요청에 따라 ATR 배수 기본값을 `3.0`에서 `2.0`으로 변경했다.

### 추가 보완 14 (2026-06-12)
- 샹들리에 라인을 Long/Short 모두 계산하도록 확장하고, 화면에서 표시 대상을 선택할 수 있는 옵션을 추가했다.

### 추가 보완 15 (2026-06-12)
- 이동평균선(MA) 표시를 선택형으로 확장했다(5, 10, 20, 60, 120, 200).
- MA 기본 선택값은 사용자 요청에 따라 20일선만 활성화하도록 변경했다.

### 추가 보완 16 (2026-06-12)
- Streamlit 조회 기본 기간을 최근 1년에서 최근 5년으로 확장했다.

### 추가 보완 17 (2026-06-12)
- 차트는 5년치 데이터를 로드하되, 초기 화면은 최근 1년만 보이도록 설정했다. 확대/축소 시 과거 5년 데이터를 탐색할 수 있다.

### 추가 보완 18 (2026-06-12)
- Streamlit 차트에 일봉/주봉/월봉 토글을 추가했다.
- 주봉/월봉은 Oracle 일봉 데이터를 내부에서 리샘플링해서 그린다.
...
## Handoff — ETF 구성종목 수집 차단 대응 (2026-08-24)

### Completed
- **첫 백필이 KRX 차단으로 사실상 실패했다** (run 32636431731, workflow는 success).
  9,196건 시도 → 성공 97 / 빈 결과 9,099 / 실패 0, 3.77시간. 스냅샷 240개 중 6개.
  받은 날짜를 보면 2022-01-07~02-04 초반 5주만 정상이고 그 뒤로 전부 빈 결과였다.
  같은 날짜를 지금 다시 조회하면 정상 응답한다 → **데이터 문제가 아니라 요청량 제한**이다.
- **원인은 설계 결함이다.** 차단이 예외가 아니라 빈 DataFrame으로 오는데, 수집기가
  그걸 "상장 전"으로 세고 있었다. 그래서 실패율 가드(50%)가 끝까지 0으로 무력화됐고
  조기 중단도 없었다.
- 고친 것:
  - **탐침(probe)** — 빈 결과가 연달으면 반드시 데이터가 있는 조합을 찔러 차단을 확정한다.
    휴장일처럼 그날만 비는 경우와 구분된다.
  - **쿨다운 후 재시도** — 차단되면 쉬었다 탐침을 다시 던지고, 회복되면 이어간다.
  - **상장 여부 사전 확인** — `list_etf_tickers`로 그날 상장된 ETF만 묻는다.
    호출이 25% 줄고, 빈 결과가 드물어져 차단 감지가 정확해진다.
  - **기본 간격 0.2 → 0.6초**
  - **부분 진행도 릴리즈한다** — 빈 결과는 기록되지 않아 다음 실행이 다시 시도하므로,
    모은 만큼 올려야 다음 실행이 덜 일한다. 워크플로의 `if: success()` 주석도 고쳤다.

### 실측 (2026-08-24, 수정본)
- 2022-01-01~02-11 구간, 상한 80회: **성공 80 / 빈 결과 0 / 실패 0** (상장 전 89건 스킵)
- 릴리즈 이어받기: 기존 97건 건너뛰고 27건만 조회 → 3,781행 / 7개 시점으로 누적
- **기존 데이터가 살아 있고 이어받는 것을 확인했다.**

### In progress
없음. 워크플로를 여러 번 돌려 240개 시점을 채우면 된다.

### Next 3 concrete tasks
1. `Collect ETF Components`를 **여러 번** 수동 실행해 백필을 완성한다. 한 번에 다
   못 받는 것을 전제로 설계했다 — 매 실행이 진행분을 릴리즈하고 다음 실행이 이어받는다.
   `max_calls`를 1,000~2,000으로 나눠 돌리면 차단 위험이 준다.
2. 차단 임계값을 실측해 `--sleep` 기본값을 조정한다. 지금 0.6초는 근거 없는 보수값이다.
3. `ETF_COMPONENT` Oracle upsert.

### Risks/blockers
- **KRX 차단 임계값을 여전히 모른다.** 0.2초 간격에서 ~90건에 걸렸다는 것만 안다.
  0.6초가 충분한지는 실측해야 한다.
- 차단이 길게 지속되면 여러 번 실행해도 진도가 안 나갈 수 있다. 그때는 `--sleep`을
  더 올리거나 하루에 나눠 돌려야 한다.

### Commands used for verification
- `python -m pytest tests/ -q` → 33 passed
- `python scripts/collect_etf_components.py --start-date 2022-01-01 --end-date 2022-02-11 --no-resume --max-calls 80`
- `python scripts/collect_etf_components.py --start-date 2022-01-01 --end-date 2022-02-11 --max-calls 30` (릴리즈 이어받기)
